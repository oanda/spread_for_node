...
function sendUsername(client, username) {
    ...
    logger.debug("sendUsername(" + username + ")");
...
var SpreadToolkit = function(options) {
    ...
    // Pass this in for testing
    this.socketConstructor = options.Socket || net.Socket;

    this.timeout = options.timeout || 60000;   // 60 seconds
    logger.trace('timeout: ' + this.timeout);
};
util.inherits(SpreadToolkit, EventEmitter);

SpreadToolkit.prototype._onData = function(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        if (this.headerOffset < this.headerLength) {
            ...
            if (this.spreadHeaderOffset === this.spreadHeaderLength) {
                ...
                logger.trace("sameEndian: " + sameEndian(this.serviceType));
                ...
                logger.trace("groups: " + numberOfGroups + ", length: " + dataLength);
            ...
            if (this.spreadDataOffset === this.spreadDataLength) {
                ...
                if (this.serviceType == 4) {
                    ...
                    logger.error("skipping unsupported service type: " + this.serviceType);
                }
...
SpreadToolkit.prototype._setup = function() {
    this.socket = new this.socketConstructor();
    ...
    this.socket.on('connect', function() {
        logger.info("Connected: " + util.inspect(this.socket));
